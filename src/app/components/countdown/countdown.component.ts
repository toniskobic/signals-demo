...
export class CountdownComponent {
  ...
  play() {
    this.endDate = moment(new Date()).add(this.timer().minutes, 'm').add(this.timer().seconds, 's').toDate();
  ...
  calculateTimeLeft() {
    if (this.endDate) {
      ...
      if (diff > 0) {
        ...
        this.timer.mutate((timer) => { timer.minutes = minutes, timer.seconds = seconds });
      } else {
